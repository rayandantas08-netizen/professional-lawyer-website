...
export default function Header() {
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <a href="#home" onClick={(e) => handleClick(e, '#home')} className="flex items-center gap-3">
            <div className="w-10 h-10 bg-secondary/20 rounded-lg flex items-center justify-center border border-secondary/30">
          ...
          <nav className="hidden lg:flex items-center gap-8">
            {navLinks.map((link) => (
              <a
                ...
                className="text-sm font-medium text-white/80 hover:text-secondary transition-colors duration-300 relative after:content-[''] after:absolute after:bottom-0 after:left-0 after:w-0 after:h-[2px] after:bg-secondary after:transition-all after:duration-300 hover:after:w-full"
              >
                {link.label}
              </a>
            ))}
      ...
      >
        <div className="bg-primary border-t border-secondary/20 px-4 py-4 space-y-1">
          {navLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
              onClick={(e) => handleClick(e, link.href)}
              className="block py-3 px-4 text-white/80 hover:text-secondary hover:bg-white/5 rounded-lg transition-all duration-200 font-medium"
            >
              {link.label}
            </a>
          ))}
